...
class PPOKLPenaltyTrainer:
    def __init__(self, model, sl_model, device, lr=1e-5, kl_beta=0.05, ppo_epochs=4):
        self.device = device
...
def save_rl_training_curve(loss_history, reward_history, entropy_history, chart_path='rl_training_curve_phase2_v2.png'):
...
# ==========================================
# 6. メイン実行スクリプト Phase 2
# ==========================================
if __name__ == '__main__':
    mp.set_start_method('spawn', force=True)
    
    print("="*60)
    print("🚀 PPO Multi-Agent Self-Play V2 - Phase 2 (Fine-tuning)") 
    print("="*60)
    ...
    # Phase 2: Phase 1 のベストモデルを読み込む (Load Phase 1 best model)
    base_policy_path = "smart_mahjong_ppo_best_v2.pth"
    if os.path.exists(base_policy_path):
        ...
        print(f" -> [Info] RLフェーズ1の最良ポリシーを読み込みました: {base_policy_path}")
    else:
    ...
    # Phase 2: 学習率を 1e-5 に減衰して微調整 (Fine-tuning LR)
    trainer = PPOKLPenaltyTrainer(model, sl_base_model, device, lr=1e-5, kl_beta=0.05)
    ...
    try:
        for it in range(1, TOTAL_ITERATIONS + 1):
            ...
            if avg_reward > best_avg_reward:
                ...
                best_path = "smart_mahjong_ppo_best_phase2_v2.pth"
        ...
        save_rl_training_curve(ppo_loss_history, reward_history, entropy_history, chart_path='rl_training_curve_phase2_v2.png')
        print("🎉 V2 Phase 2 非同期自己対局パイプラインの実行が完了しました。(Pipeline finished.)")